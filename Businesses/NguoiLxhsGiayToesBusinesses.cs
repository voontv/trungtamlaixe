...
{
    [ImplementBy(typeof(NguoiLxhsGiayToesBusinesses))]
    public interface INguoiLxhsGiayToesBusinesses
    {
        Task<KhoaHoc> PostKhoaHoc(KhoaHocCreateRequest khoaHoc);
    }

    public class NguoiLxhsGiayToesBusinesses: ControllerBase, INguoiLxhsGiayToesBusinesses
    {
        private readonly GplxCsdtContext _context;
        private readonly IAuthenInfo _authenInfo;
        public NguoiLxhsGiayToesBusinesses(GplxCsdtContext context, IAuthenInfo authenInfo)
        {
            _context = context;
